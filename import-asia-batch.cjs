...
if (!TARGET_DIR || !fs.existsSync(TARGET_DIR)) {
  ...
  console.log('Uso: node import-asia-batch.cjs "C:\\Users\\HP\\Desktop\\cvs asia\\asia_pois_csv"');
  process.exit(1);
}
...
async function startBatch() {
  console.log(`Avvio importazione batch da cartella: ${TARGET_DIR}`);
  const files = fs.readdirSync(TARGET_DIR).filter(f => f.endsWith('.csv'));
  let totalInserted = 0;
  let totalGems = 0;
  
  for (const f of files) {
    const res = await processFile(path.join(TARGET_DIR, f));
    if (res) {
