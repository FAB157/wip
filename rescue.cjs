...
const regex = /(const fallbackInterests = interests \|\| \[\];[\s\S]*?if \(specialRequests\) \{[\s\S]*?\})[\s\S]*?(app\.post\("\/api\/groq\/itinerary-stream", rateLimiter, async \(req, res\) => \{)/;
...
if (match) {
  const newContent = c.substring(0, match.index) + match[1] + missingBlock + match[2] + c.substring(match.index + match[0].length);
