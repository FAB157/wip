...
const missingBlock = `        console.log(\`[Viator] Fetching real tours for itinerary in \${destination}...\`);
...
1. LUNGHEZZA E DETTAGLI: "attivita" e "consiglio_guida" (Nicky e Dante) DEVONO essere paragrafi corposi di ALMENO 5 o 6 RIGHE (100-150 parole). Vieta i riassuntini.
2. CONSIGLI DOPPI ESTESI: Inserisci ENTRAMBE le guide (Nicky e Dante), con dettagli storici profondi e posizioni foto specifiche.
3. TABELLA BUDGET ESTESA: Ogni voce DEVE estendersi per ALMENO 2 righe e DEVE includere consigli specifici sui piatti locali da ordinare (es. Bicerin).
4. INFO VIAGGIO BLINDATE: Le 4 sezioni (precauzioni, suggerimenti, raccomandazioni, zone_da_evitare) DEVONO avere 3 voci lunghe ciascuna. USA SOLO nomi propri e vie esatte.
5. RITMO E TIMING: \${ritmoTimingRule}\`;

      if (pois && pois.length > 0) {
        prompt = \`Crea un itinerario ottimizzato per \${days} giorni a \${destination} (dalle \${tInizio} alle \${tFine}) includendo: \${pois.join(", ")}.\${hardRules}\`;
      } else {
        const fallbackInterests = interests || [];
        prompt = \`Crea un itinerario ottimizzato per \${days} giorni a \${destination} (dalle \${tInizio} alle \${tFine}) basato su: \${fallbackInterests.join(", ")}.\${hardRules}\`;
      }

      if (specialRequests) prompt += \`\\nRICHIESTE PARTICOLARI UTENTE: "\${specialRequests}". Adattati rigorosamente.\`;`;

const idx = c.indexOf("if (specialRequests) prompt += `\\nRICHIESTE PARTICOLARI UTENTE");
if (idx !== -1) {
  c = c.substring(0, idx) + missingBlock + "\n" + c.substring(idx);
